...
class Degree {
    ...
    companion object {
        ...
        private val pattern =
            Pattern.compile("^([+|−|-|–|-])(\\d+)[°]\\s(\\d+)[′|']\\s(\\d+[.]*\\d*)$") // for:  −11° 09′ 40.5
        ...
        fun arcSin(x: Double): Double =
            when {
                x > 1 -> 90.0
                x < -1 -> -90.0
                else -> Math.toDegrees(kotlin.math.asin(x))
            }
        ...
        fun arcCos(x: Double): Double =
            when {
                x > 1 -> 0.0
                x < -1 -> 0.0
                else -> Math.toDegrees(kotlin.math.acos(x))
            }
